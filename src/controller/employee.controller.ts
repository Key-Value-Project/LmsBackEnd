...
class EmployeeController {
    ...
    public getEmployeeById = async (req: Request, res: Response, next: NextFunction) => {
        ...
        try {
            ...
            if (!employee) {
                throw new HttpException(404, 'Record not found', ['Employee not found in the database for the given id']);
            }
    ...
    public createEmployee = async (req: RequestWithUser, res: Response, next: NextFunction) => {
        try {
            Permission.userPermission(req, [Role.HR, Role.ADMIN], ['You are not authorized to create an employee']);
    ...
    public deleteEmployee = async (req: RequestWithUser, res: Response, next: NextFunction) => {
        const { id } = req.params;
        try {
            Permission.userPermission(req, [Role.HR, Role.ADMIN], ['You are not authorized to delete an employee']);
            await this.employeeService.deleteEmployee(Number(id));
            res.status(200).json({ message: `Employee with id: ${id} deleted successfully` });
        } catch (err) {
            console.log(err);
            if (err.status !== 403) {
                const error = new HttpException(404, 'Record not found', ['Employee not found in the database for the given id']);
                next(error);
            } else {
                next(err);
            }
    ...
    public updateEmployeePassword = async (req: RequestWithUser, res: Response, next: NextFunction) => {
        ...
        try {
            ...
            if (!updatedEmployee) {
                throw new HttpException(404, 'Record not found', ['Employee not found or password is incorrect']);
            }
